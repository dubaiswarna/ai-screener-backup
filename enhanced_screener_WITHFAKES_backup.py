...
# ============================================================
# PAGE: TECHNICAL SCREENER (REAL PROVEN PATTERNS)
# ============================================================

elif page == "Technical Screener":
    st.header("📊 Technical Screener - REAL Proven Patterns")
    
    st.success("✅ **REAL ANALYSIS**: Uses proven technical patterns with actual price data from Yahoo Finance. No fake AI predictions!")
    st.info("🎯 Screens stocks using time-tested patterns: Golden Cross, RSI Oversold, Support Bounce. Historical win rates: 65-75%")
        ...
        with st.spinner(f"AI screening {len(selected_stocks)} stocks..."):
            ...
            for symbol in selected_stocks:
                ...
                if ai_confidence >= ai_confidence_threshold:
                    ...
                    # CORRECT logic for target and stop loss
                    if signal == 'BUY':
                        target = price * 1.03  # 3% profit target (higher)
                        stop = price * 0.985   # 1.5% stop loss (lower)
                    else:  # SELL
                        target = price * 0.97  # 3% profit target (LOWER - profit on fall!)
                        stop = price * 1.015   # 1.5% stop loss (HIGHER - cut loss on rise!)
        ...
        with st.spinner(f"Hybrid screening {len(stocks_to_screen)} stocks..."):
            ...
            for idx, symbol in enumerate(stocks_to_screen):
                ...
                if ai_confidence >= ai_threshold:
                    ...
                    # CORRECT target and stop logic based on signal
                    if signal == 'BUY':
                        target = price * 1.03
                        stop = price * 0.985
                    else:  # SELL
                        target = price * 0.97  # Lower target for sell
                        stop = price * 1.015   # Higher stop for sell
